...
test('current answer',()=>{
    render(<Robo currentAnswer="Answer1" previousQueries={[]} invalidQuestion="" inputValue="Answer1"/>);
...
test('previous answered question heading to be present',()=>{
    ...
    render(<Robo currentAnswer="" previousQueries={mockPrev} invalidQuestion="" inputValue=""/>);
...
test('previous answered question list to be present',()=>{
    ...
    let prevAnswersList = screen.getAllByRole('listitem');
        ...
        expect(prevAnswersList[0]).toHaveTextContent('Question:whats your name - Answer: RoboBob');
 })
 test('invalid question scenario',()=>{
    render(<Robo currentAnswer="" previousQueries={[]} invalidQuestion="Invalid" inputValue="Invalid"/>);
